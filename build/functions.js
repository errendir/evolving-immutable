...
export const memoizeForSlots = ({ computeSlot, executeFunction }) => {
    return semiPureFunction({
        createMemory: () => ({
            ...
            functionInstanceBySlot: Map().asMutable(),
        }),
...
export const memoizeForRecentArgumentObject = memoize((currentArguments, pastArguments) => {
    ...
    if (currentNames.length !== pastNames.length)
        return false;
    for (let i = 0; i < currentNames.length; ++i) {
        const name = currentNames[i];
        if (currentArgumentObject[name] !== pastArgumentObject[name])
            return false;
    }
    return true;
});
export function semiPureFunction({ createMemory, executeFunction }) {
...
export const composeFunctions = (...functions) => {
    return semiPureFunction({
        ...
        executeFunction: ({ functionInstances: [firstFunctionInstance, ...restOfFunctionInstances] }, ...args) => {
            return restOfFunctionInstances.reduce((result, functionInstance) => functionInstance(result), firstFunctionInstance(...args));
        }
    });
};
