...
export function wrapDiffProcessor(diffProcessorFactory, { inSet = false, outSet = false } = {}) {
    ...
    const apply = (newArgument) => {
        newArgument.diffFromCallbacks(currentArgument, inDiffProcessor);
...
export function wrapDualDiffProcessor(diffProcessorFactory) {
    ...
    const apply = (newLeftArgument, newRightArgument) => {
        newLeftArgument.diffFromCallbacks(currentLeftArgument, inDiffProcessor[0]);
        currentLeftArgument = newLeftArgument;
        newRightArgument.diffFromCallbacks(currentRightArgument, inDiffProcessor[1]);
